...
class Message:
  ...
  def __init__(self, name, arguments, expects_ack, expects_reply, is_reply):
    ...
    assert type(arguments) == types.ListType
    self.name = name
...
def _cmsg(name, num_args, expects_ack=False, expects_reply=False, is_reply=None):
...
def _amsg(name, num_args, expects_ack=False, expects_reply=False, is_reply=None):
  "Alice sends these to Switzerland"
  msg = Message(name, num_args, expects_ack, expects_reply, is_reply)
  alice_out_messages[name] = msg
  switzerland_in_messages[name] = msg

def _smsg(name, num_args, expects_ack=False, expects_reply=False, is_reply=None, assert_len=None):
...
_amsg("parameters", [{"clock dispersion" : 0.085, 
               "will send actual packets": False,  
               }])
